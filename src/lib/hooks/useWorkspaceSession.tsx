 ...
 * @example
 * ```tsx
 * const { workspaceId, role, canWrite } = useWorkspaceSession();
 * ```
 */
export function useWorkspaceSession(): WorkspaceContextValue {
  const context = useContext(WorkspaceContext);

  if (!context) {
    throw new Error(
      "useWorkspaceSession must be used within a WorkspaceProvider. " +
