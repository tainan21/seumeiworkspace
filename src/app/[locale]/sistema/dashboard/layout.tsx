...
import type { WorkspaceLayoutContract, MenuItem, TopBarVariant, BrandColors } from "~/types/workspace"
import { WorkspaceLayoutProvider, useWorkspaceLayout } from "~/lib/hooks/use-workspace-layout"
...
function DashboardLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const { workspace, theme, colors, isLoading } = useWorkspaceLayout()
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  
  // Extrair locale do pathname (formato: /[locale]/sistema/dashboard/...)
  const locale = pathname.split("/")[1] || "pt"
  const activeItem = pathname.includes("/sistema/dashboard") && pathname.split("/").length === 4 ? "dashboard" : pathname.split("/").filter(Boolean).pop() || "dashboard"

  useEffect(() => {
    if (!isLoading && !workspace) {
      router.push(`/${locale}/sistema/onboarding`)
    }
  }, [isLoading, workspace, router, locale])
          ...
          <SidebarItem
            item={
              workspace.menuItems.find((i) => i.id === "settings") || {
                ...
                icon: "Settings",
                order: 99,
                route: "/sistema/dashboard/settings/components",
              }
            }
...
function SidebarItem({
  ...
  return (
    ...
    >
      <Link
        href={`/${locale}/sistema${item.route}`}
        className={cn(
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem asChild>
                <Link href={`/${locale}/sistema/dashboard/settings/components`}>Configurações</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link href={`/${locale}/sistema/dashboard/settings/layout-builder`}>
