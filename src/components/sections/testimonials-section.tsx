...
export function TestimonialsSection() {
  ...
  const testimonials = (Array.isArray(testimonialsData) ? testimonialsData : []).map((item, index) => ({
    ...
    ][index] || "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&h=150&fit=crop&crop=face",
  }))
  ...
  const companiesData = (t as any)("testimonials.companies") as string[] | undefined
  const logos = Array.isArray(companiesData) ? companiesData : []
  return (
    <section id="testimonials" className="px-6 py-24 bg-zinc-900/30">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="border border-zinc-800 py-1.5 px-4 rounded-full text-sm text-zinc-400">{tScoped("sectionTag")}</div>
        ...
        <div className="mt-16 pt-16 border-t border-zinc-800/50">
          <p className="text-center text-sm text-zinc-500 mb-8">{tScoped("trustedBy")}</p>
