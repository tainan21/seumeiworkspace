// ============================================
// DOMAIN: Workspace
// Todas as funções e regras de workspace
// ============================================
...
import { validateCompanyIdentifier } from "../company/company"
import { applyTemplateCompatibility, chooseDefaultAppsByCompanyType } from "../template/template"
import { generateRBACDefaults } from "../rbac/rbac"
import { SYSTEM_MENU_ITEMS } from "@/lib/mock-data/components"
...
export function enforceSingleFreeWorkspace(
  ...
  const userWorkspaces = existingWorkspaces.filter((w) => w.ownerId === userId || w.createdBy === userId)
  ...
  const hasFreeWorkspace = userWorkspaces.some((w) => w.settings.billingPlan === "free")
 ...
 */
export function initializeAnalyticsSettings(billingPlan: BillingPlan): WorkspaceAnalyticsSettings {
  return { ...DEFAULT_ANALYTICS_SETTINGS[billingPlan] }
}

/**
 * Atualiza configurações de analytics com validação de limites por plano
 */
export function updateAnalyticsSettings(
  current: WorkspaceAnalyticsSettings,
  updates: Partial<WorkspaceAnalyticsSettings>,
  billingPlan: BillingPlan,
): { success: true; settings: WorkspaceAnalyticsSettings } | { success: false; error: string } {
  const planLimits = DEFAULT_ANALYTICS_SETTINGS[billingPlan]

  if (updates.retentionDays !== undefined) {
    if (updates.retentionDays > planLimits.retentionDays) {
      return {
        success: false,
        error: `Plano ${billingPlan} permite no máximo ${planLimits.retentionDays} dias de retenção`,
      }
    }
    if (updates.retentionDays < 7) {
      return {
        success: false,
        error: "Retenção mínima é de 7 dias",
      }
    }
  }

  if (billingPlan === "free") {
    if (updates.trackNavigation === true || updates.trackSearches === true) {
      return {
        success: false,
        error: "Tracking de navegação e busca requer plano Pro ou superior",
      }
    }
  }
...
/**
 * Verifica se workspace pode usar analytics
 */
export function isAnalyticsEnabled(workspace: Workspace): boolean {
  return workspace.settings.analytics?.enabled ?? true
}

/**
 * Verifica se um tipo específico de tracking está habilitado
 */
export function isTrackingEnabled(
  workspace: Workspace,
  trackingType: "pageViews" | "featureUsage" | "navigation" | "searches" | "errors",
): boolean {
  if (!isAnalyticsEnabled(workspace)) return false

  const analytics = workspace.settings.analytics
  if (!analytics) return trackingType === "pageViews" || trackingType === "featureUsage" || trackingType === "errors"

  switch (trackingType) {
    case "pageViews":
      return analytics.trackPageViews
    case "featureUsage":
      return analytics.trackFeatureUsage
    case "navigation":
      return analytics.trackNavigation
    case "searches":
      return analytics.trackSearches
    case "errors":
      return analytics.trackErrors
    default:
      return false
  }
}
...
export function isSystemItem(itemId: string): boolean {
...
export function assembleMenu(componentsSelected: MenuComponent[], appsEnabled: string[]): MenuItem[] {
  ...
  componentsSelected.filter((c) => c.id !== "dashboard" && c.id !== "settings").forEach((c) => processComponent(c))
...
export async function createWorkspace(
  ...
  const limitCheck = enforceSingleFreeWorkspace(input.userId, context.existingWorkspaces, context.userPlan)
  ...
  if (input.company.identifier?.value) {
    const idValidation = validateCompanyIdentifier(input.company.identifier.value, input.company.identifier.type)
  ...
  if (input.template) {
    ...
    if (template) {
      const compat = applyTemplateCompatibility(template, input.selectedFeatures, input.companyType)
  ...
  const appRecommendations = chooseDefaultAppsByCompanyType(input.companyType, input.employeeCount)
